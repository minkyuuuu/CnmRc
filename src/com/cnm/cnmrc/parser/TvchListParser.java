...
public class TvchListParser {

	// URL
	private String URL = "";

	private ItemTvchListList list;

	public TvchListParser(String url) {
		list = new ItemTvchListList();
		URL = url;
	}
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				...
				case XmlPullParser.START_TAG:
					...
					if (tag.equalsIgnoreCase("resultCode")) {
						...
						list.getList().add(new ItemTvchList()); 		// 리스트 추가
						mCurrentCount = list.getList().size() - 1; 	// 현재 추가된 리스트의 위치 반환

					} else if (tag.equalsIgnoreCase("genreCode")) {
						eventType = parser.next();
						if (parser.getText() != null) list.getList().get(mCurrentCount).setGenreCode(parser.getText());
						parser.next();
					} else if (tag.equalsIgnoreCase("genreName")) {
						eventType = parser.next();
						if (parser.getText() != null) list.getList().get(mCurrentCount).setGenreName(parser.getText());
						parser.next();
	...
	public ItemTvchListList getList() {
		return list;
	}
}
